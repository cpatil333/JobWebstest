...
export const applyJob = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Job applied successful", success: true });
  } catch (error) {
    console.log(error);
  }
};
...
export const getAppliedJobs = async (req, res) => {
  try {
    ...
    //no exists application
    if (!appliction) {
      return res.status(400).json({ message: "No Applicant", success: false });
    }

    return res.status(200).json({ appliction, success: true });
  } catch (error) {
    console.log(error);
  }
};
...
export const getApplicants = async (req, res) => {
  try {
    ...
    //if no jobs found
    if (!job) {
      return res.status(400).json({ message: "Job tot found", success: false });
    }
